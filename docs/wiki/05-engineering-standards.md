...
How we keep MiCyte Portal lean and correct. This is the operating contract for
...
- **L1 CORE** — `micyte/core/` — the lean MOS datum-database
  library. Pure domain logic, no I/O, no framework.
- **L2 SURFACE** — `micyte/ports/` (pure contracts) +
  `fnd_app/packages/adapters/` (implementations: SQL, filesystem, AWS).
- **L3 UI** — `micyte/tools/`, `micyte/state_machine/`,
  and the Flask host + JS shell under
  `fnd_app/instances/_shared/portal_host/`.
...
`fnd_app/tests/architecture/`, so the standards are mechanical, not matters of
taste.
...
`fnd_app/tests/architecture/`. Each one parses every `.py` file in a package
...
- `fnd_app/tests/architecture/test_core_datum_refs_boundaries.py:42` — asserts
  `micyte/core/datum_refs` imports only stdlib + `micyte.core`,
  and that the source contains no instance/runtime path tokens.
- `fnd_app/tests/architecture/test_state_machine_boundaries.py:47` — confines
  `micyte/state_machine` to `core` + `state_machine` imports and bans
  dynamic-import / glob tokens.
- `fnd_app/tests/architecture/test_core_datum_rules_boundaries.py` — same
  pattern for `micyte/core/datum_rules`.
- `fnd_app/tests/architecture/test_datum_store_port_boundaries.py` and
  `fnd_app/tests/architecture/test_filesystem_adapter_boundaries.py` — pin the
  port/adapter seam.
- `fnd_app/tests/architecture/test_no_disk_datum_authorities.py` and
  `fnd_app/tests/architecture/test_no_filesystem_datum_authority_in_runtime.py`
  — keep the MOS-is-canonical rule honest (no on-disk datum authorities).
- `fnd_app/tests/architecture/test_palette_eligibility_purity.py` — purity of
...
`micyte/adapters/sql/datum_semantics.py`) is the *real*
address / hyphae / MSS engine and is not actually SQL-coupled — it imports only
`micyte.ports.datum_store` and a local JSON helper
...
`fnd_app/tests/architecture/test_core_datum_ops_boundaries.py`.** Every other
...
The bar for "this is worth deleting" is already set by a multi-phase
bloat-reduction program: an audit framing, concrete findings turned into work,
and a remediation execution pass. Weigh large additions against that precedent —
duplicate-definition pressure especially, which is itself test-guarded:
`fnd_app/tests/architecture/test_no_duplicate_definitions.py` and
`fnd_app/tests/architecture/test_canonical_document_naming.py` exist to catch
copy-paste drift.
...
- **Module-level catalog cache.**
  `micyte/adapters/sql/datum_store.py:108` declares
...
- **Efficient single-document update path.**
  ...
  (`micyte/adapters/sql/datum_store.py:296`) replaces exactly one
...
`micyte/tools/workbench_ui/service.py` (~992 LOC) re-derives its
...
`fnd_app/tests/` is organized by *kind of guarantee*, not by feature:
...
```bash
python -m pytest fnd_app/tests/architecture -q
```
...
(`fnd_app/instances/_shared/portal_host/app.py:426`, policy block at
